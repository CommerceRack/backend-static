...
//				alert(MYADD2CART_pogs[i]['optional']+" and pogvalue = "+pogValue);
				...
				pogValue = document.addToCartFrm['pog_'+pogid].value;
				}


//If the option IS required (not set to optional) AND the option value is blank, record an error

			if(MYADD2CART_pogs[i]['optional'] != 1 && pogValue == "")	{
				valid = false;
				$('JSONpogErrors').innerHTML +=	 "The choice for '"+MYADD2CART_pogs[i]['prompt']+"' is required. Please make a selection <span class='zhint' style='font-weight:normal'>( id: "+pogid+")</span><br>";
				}
...
//adds options to the select list.
    while (i < len) {
		...
		if(pog['options'][i]['p'])
			selOption.innerHTML +=   handlePogPrice(pog['options'][i]['p']); //' '+pog['options'][i]['p'][0]+'$'+pog['options'][i]['p'].substr(1);
		selectList.appendChild(selOption);
		i++;
       }
...
//adds options to the select list.
    while (i < len) {
		...
		if(pog['options'][i]['p'])
			selOption.innerHTML += ' '+pog['options'][i]['p'][0]+'$'+pog['options'][i]['p'].substr(1);
		selectList.appendChild(selOption);
		i++;
       }
...
//the price gets passed as +5.00 or -5.00 but needs to be displayed at +$5.00 or -$5.00, hence then [0] and substr crap below.
		if(pog['options'][i]['p'])
			radioLabel.appendChild(document.createTextNode(' '+pog['options'][i]['p'][0]+'$'+pog['options'][i]['p'].substr(1)));  
...
//Creates the 'hidden input' form field in the DOM.	
	...
	with(textbox) {
		setAttribute("id", "pog_"+pogid);
		setAttribute("value", pog['default']);
		setAttribute("name", "pog_"+pogid);
		setAttribute("type", "hidden");
...
//Creates the 'text input' form field in the DOM.	
	...
	with(textbox) {
		...
		setAttribute("maxlength", pog['maxlength']); // this doesn't work in IE... so there's a js function added to each onkeypress
...
//Creates the 'text input' form field in the DOM.	
	...
	with(textbox) {
		...
		setAttribute("name", "pog_"+pogid);
		className = "zform_textbox";
		setAttribute("maxlength", pog['maxlength']);  // this doesn't work in IE. The onkeyup and down below are the workaround.
		setAttribute("type", "text");
		}
