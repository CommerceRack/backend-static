...
Change Log
2010-12-13
 - changed the inventory check to be < 1 instead of == 0 to handle negative inventory. 
 	(didn't rename file because this is a bug fix and needed to be retroactive.)

2010-11-08
 - stripped out 'flags' as it wasn't fully implemented.
 - added simple inventory check to select, radio and image select. (if there is only one inventory-able option group, don't show options with 0 inventory.
 - commented out the magicthumb typeof. wasn't working (magicthumb. is the class). just set MTB=MT if you want to enable magicthumb


2010-08-12
 - updated validation script so that skips for attribs and optional options happens earlier. More efficient and allows for easier customization user-to-user.
...
2009-10-04 - Updated to make placement of pogs more flexible. 2 divs now required:
...
//reset displayed errors and the stid.
...
//if the pog var is set, loop through it and validate.  
	if(MYADD2CART_pogs)	{
		...
		for(i = 0; i < MYADD2CART_pogs.length; i++)	{
			...
			else if (MYADD2CART_pogs[i]['type'] == 'attribs'){
				//if the pog type is attrib (finder), validation isn't needed.			
				}
...
//prototype method for getting radio button value
					pogValue = $$('input:checked[type="radio"][name="pog_"+pogid]').pluck('value');  
					}
				else	{
...
//if all options are selected AND checkinventory is on, do inventory check.
	if(valid == true && simpleInvCheck == 1)	{
//		alert(thisSTID);
		if(MYADD2CART_sku[thisSTID]['inv'] < 1)	{
			$('JSONpogErrors').innerHTML = "We're sorry, but the combination of selections you've made is not available. Try changing one of the following:"+inventorySogPrompts;
			valid = false;
			}
...
countInventoryableOptions: function() {
// return the number of inventoryable options. Inventory will be set to 1 or 0.
	c = 0;
	for ( var i=0, len=this.pogs.length; i<len; ++i )	{
		c = c + (this.pogs[i].inv * 1);  //multiple by one to treat as integer.
		}
	return(c);
	},
...
//adds options to the select list.
    while (i < len) {
//hide options where there is only 1 inventory-able group and the quantity is zero
		if(pog['inv'] == 1 && numInvOptions == 1 && MYADD2CART_sku[sku+":"+pogid+pog['options'][i]['v']]["inv"] < 1)	{
			  //do nothing.  'continue' will break because i is incremented at the end of the loop.
			}
		else	{
			selOption = document.createElement("option");
			...
			if(pog['options'][i]['p'])
				selOption.innerHTML +=   handlePogPrice(pog['options'][i]['p']);
			selectList.appendChild(selOption);
...
//the price gets passed as +5.00 or -5.00 but needs to be displayed at +$5.00 or -$5.00, hence then [0] and substr crap below.
			if(pog['options'][i]['p'])
				radioLabel.appendChild(document.createTextNode(handlePogPrice(pog['options'][i]['p'])));  
