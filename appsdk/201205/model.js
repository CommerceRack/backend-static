...
//executes the callback.onError and takes into account extension. saves entire callback object into callbackObj so that it can be easily validated and executed whether in an extension or root.
						...
						if(callbackObj && typeof callbackObj.onError == 'function'){
							callbackObj.onError('It seems something went wrong. Please continue, refresh the page, or contact the site administrator if error persists. Sorry for any inconvenience. (error: most likely a request failure for all three attempts [uuid = '+uuid+'])')
							}
						}
					else	{
...
//								myControl.util.dump(" -> extension ("+namespace+") has dependencies. veryify they're loaded before executing callback");
								...
								myControl.util.dump(" -> executing callback: "+callback);
 								myControl.ext[namespace].callbacks[callback].onSuccess();
								}
							}
						else	{
//							myControl.util.dump(" -> extension "+namespace+" loaded fine but contained no callback");
				...
				error: function(a,b) {
					...
					myControl.util.dump(" -> EXTCONTROL ("+namespace+")Got to error. error type = "+b);
					}
				});
...
//namespace and filename are required for any extension.
					if(!extensionObj[i].namespace || !extensionObj[i].filename)	{
						if(extensionObj.callback)	{
							extensionObj[i].callback.onError("extension did not load because no namespace and/or filename set")
							}
