...
//				myControl.util.dump('BEGIN myControl.ext.store_crm.calls.categoryDetail.init');
				this.dispatch(tagObj);
				return 1; //1 dispatch is occuring.
				},
			dispatch : function(tagObj)	{
...
eleID was expanded to allow for an object. 
...
//creates a copy of the template.
			var r;
			if(!templateID)	{
				myControl.util.dump(" -> ERROR! templateID not specified in createTemplateInstance. eleID = "+eleID);
				r = false;
				}
			else if(myControl.templates[templateID])
				r = myControl.templates[templateID].clone();
			else	{
				myControl.util.dump(" -> ERROR! template ("+templateID+") does not exist! eleID = "+eleID);
				r = false;
				}
			if(r)	{
	...
	//					myControl.util.dump(" -> assigned a data-pid attr: "+eleID.pid);
					...
					if(eleID.stid)	{
//					myControl.util.dump(" -> assigned a data-stid attr: "+eleID.stid);
						r.attr('data-stid',eleID.stid); //this has a makesafe on it, but I think that's wrong. I think we want the unmolested sku. changed 2012-01-24
						}
					if(eleID.catsafeid)	{
	//					myControl.util.dump(" -> assigned a data-pid attr: "+eleID.pid);
				...
				r.attr('data-templateID',templateID); //lets us know later what template was used, if we need to re-render something.
...
//		if(safeTarget == 'prodReviewSummary')
...
// what's the * for? this necessary? this adding extra overhead? look in to it. ???
...
//				myControl.util.dump(' -> extension = '+bindData.extension);
//				myControl.util.dump(data);
...
//						myControl.util.dump(" -> extension specified ("+bindData.extension+") and "+bindData.format+" is a function. executing.");
						myControl.ext[bindData.extension].renderFormats[bindData.format]($focusTag,{"value":value,"bindData":bindData,"safeTarget":safeTarget,"attributes":data});
						}
...
//var htmlid = $tag.attr('id')
//if(!htmlid)	{
//	htmlid = 'random_'+Math.floor(Math.random()*10001); //wiki needs an id to be assigned to, for now.
//	$tag.attr('id',htmlid);
//	}
//myControl.util.dump('htmlid = '+htmlid);
var text = data.value;
var target = document.getElementById($tag.attr('id')); //### creole parser doesn't like dealing w/ a jquery object. fix at some point.
myCreole.parse(target, text,{},data.bindData.wikiFormats);
