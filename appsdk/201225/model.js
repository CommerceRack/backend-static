...
//			myControl.util.dump("QID = "+QID+" and L = "+L+" and immutableRequestInProgress = "+immutableRequestInProgress);
...
//tmp work around for login invalid username error.
					if(responseData.errid === 1)	{
						myControl.callbacks.handleSessionStartResponse.onError(responseData,null);
						}
					else	{
...
//an appPageGet request needs the requested data to extend the original page object. (in case two separate request come in for different attributes for the same category.	
	...
	//			myControl.util.dump(' -> no datapointer set for uuid '+uuid);
			...
			if(hasErrors)	{
				if(callback == false)	{
					myControl.util.dump('WARNING response for uuid '+uuid+' had errors. no callback ['+callback+'] was defined.')
					}
...
//						myControl.util.dump("GOT HERE!");
						...
						responseData['errmsg'] = "could not find product: "+responseData['_rcmd'].split('|')[1]+" (may not exist)";
