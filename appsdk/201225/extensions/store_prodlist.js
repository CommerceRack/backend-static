...
//allows for just the mp controls to be turned off, but will leave the rest of the header (sorting, summary, etc) enabled.
						...
						var hideMultipage = myControl.ext.store_prodlist.vars[paramObj.parentID].hide_multipage || false;
...
//					myControl.util.dump(' -> multi page product list.');
					...
					if(myControl.ext.store_prodlist.vars[parentID].hide_multipage || myControl.ext.store_prodlist.vars[parentID].hide_multipage_controls)	{
//if either multipage (entire header) or multipage controls (pagination) is turned off, don't render pagination.
						}
					else	{
						$('.mpControlContainer').empty().remove();
