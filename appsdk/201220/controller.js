...
//				myControl.util.dump('IMGSRC => '+imgSrc);
		...
		stuffList : function($tag,data)	{
			...
			for(var i = 0; i < L; i += 1)	{
				stid = data.bindData.cleanValue[i].stid;
				myControl.util.dump(" -> STID: "+stid);
				$o = myControl.renderFunctions.transmogrify({'id':'cartViewer_'+stid,'stid':stid},templateID,data.bindData.cleanValue[i])
...
//				myControl.util.dump("in for loop. key = "+key);
		...
		unix2mdy : function($tag,data)	{
			var r;
			r = myControl.util.unix2Pretty(data.bindData.cleanValue,data.bindData.showtime)
