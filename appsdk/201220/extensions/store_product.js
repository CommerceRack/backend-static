...
//				myControl.util.dump(data.bindData.cleanValue)
				var L = data.bindData.cleanValue.length;
				for(i = 0; i < L; i += 1)	{
					$tag.append(myControl.renderFunctions.transmogrify({'id':'review_'+i},templateID,data.bindData.cleanValue[i]));
					}
				return L;
				},
...
//add all the necessary fields for quantity inputs.
			atcQuantityInput : function($tag,data)	{
				if(myControl.ext.store_product.util.productIsPurchaseable(data.bindData.cleanValue))	{
					...
					o += "<input type='number' value='1' size='3' name='quantity' min='1' class='zform_number' id='quantity_"+data.bindData.cleanValue+"' />";
