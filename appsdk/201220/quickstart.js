...
//	myControl.util.dump(window.location.hash);
...
//				myControl.util.dump(tagObj);
				...
				tagObj.pid = myControl.data[tagObj.datapointer].pid;
...
//a category page gets translated. A product page does not because the bulk of the product data has already been output. prodlists are being handled via buildProdlist
					...
					tmp = myControl.data['appProductGet|'+tagObj.pid]
					if(typeof myControl.data['appReviewsList|'+tagObj.pid] == 'object')	{
						tmp['reviews'] = myControl.ext.store_product.util.summarizeReviews(tagObj.pid); //generates a summary object (total, average)
						tmp['reviews']['@reviews'] = myControl.data['appReviewsList|'+tagObj.pid] ['@reviews']
						}
...
//				myControl.util.dump(data);
				...
				if(data.bindData.before)	{
					$tag.before(data.bindData.before);
					}
				if(data.bindData.after)	{
					$tag.after(data.bindData.after);
					}
				if(data.bindData.wrap)	{
					$tag.after(data.bindData.wrap);
					}
				},
			...
			banner : function($tag, data)	{
				var tmp = decodeURI(data.bindData.cleanValue);
...
//				myControl.util.dump(infoObj);
				...
				case 'product':
					myControl.ext.myRIA.vars.user.recentlyViewedItems.push(infoObj.pid);
				...
				case 'homepage':
					...
					break;
				case 'category':
					myControl.ext.myRIA.vars.user.recentCategories.push(infoObj.navcat);
					myControl.ext.myRIA.util.showPage(infoObj);
					break;

				case 'search':
//					myControl.util.dump(" -> Got to search case.");
					$('#headerKeywordsInput').val(infoObj.keywords);
...
// will return either the safe path or pid or something else useful
			...
			handleFormField : function(thefield)	{
...
//bindData is passed into buildProdlist so that any supported prodlistvar can be set within the data-bind. (ex: withInventory = 1)
					...
					bindData.csv = myControl.data['appProductGet|'+P.pid]['%attribs'][attribute]
					numRequests += myControl.ext.store_prodlist.util.buildProductList(bindData)
					
//					numRequests += myControl.ext.store_prodlist.util.getProductDataForLaterUse(myControl.data['appProductGet|'+P.pid]['%attribs'][attribute].split(','),'mutable');
					}
				}
...
return r;				
			...
			createTemplateFunctions : function()	{
				...
				var pageTemplates = new Array('categoryTemplate','productTemplate','companyTemplate','customerTemplate','homepageTemplate','searchTemplate');
...
//in some cases, such as showContent/oninits, we may not 'know' what template is being loaded when this code is executed. try to guess.
				if(!P.templateID)	{
					...
					if(typeof myControl.templates[couldBeType+"Template"] == 'object')	{
						myControl.util.dump(" -> Guessing template: "+couldBeType+"Template");
				...
				if(P.templateID && P.state && typeof myControl.ext.myRIA.template[P.templateID][P.state] == 'object')	{
					...
					myControl.util.dump(" -> typeof template:"+ typeof myControl.ext.myRIA.template[P.templateID][P.state]);
					myControl.util.dump(P);
					}
