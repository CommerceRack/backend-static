...
var ajaxRequest = myControl.model.loadRemoteTemplates("201219/admin_templates.html");
...
ajaxRequest.error(function(d,e,f){
	...
	$('#globalMessaging').append(myControl.util.formatMessage("Uh oh! Something bad happened. If the error persists, please contact Zoovy technical support. error: could not load admin templates. (dev - see console for more details)"));
...
//admin templates are added to their own div so that .html() can be used without impacting any default templates that may not have loaded.
	var $adminTemps = $('#adminTemplates');
	if($adminTemps.length == 0)	{
		$adminTemps = $("<div />").attr('id','adminTemplates').hide().appendTo('body');
		}
	$adminTemps.html(data);
	var templateErrors = myControl.model.loadTemplates(['adminProdStdForList','adminProdSimpleForList','adminElasticResult','adminProductFinder','adminMultiPage','adminOrderLineItem','orderDetailsTemplate','orderStuffItemTemplate']);
	if(templateErrors)	{
		myControl.util.formatMessage(templateErrors);
