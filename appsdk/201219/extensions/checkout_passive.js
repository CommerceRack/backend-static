...
//nuke old form content. not needed anymore. gets replaced with invoice-ish content.
				...
				$('.itemCount').text('0'); //used in minicarts.
...
*/
				$('.paymentRequired').append(myControl.data[tagObj.datapointer].payment_status_detail).find('a').click(function(event){event.preventDefault();}); //this bind goes over an href. the bind needs priority since the href is wrong.;
...
//				https://ssl.zoovy.com/s=sporks.zoovy.com/customer/order/status?cartid=SESSION&orderid=data.value
				$tag.click(function(event){
					window.location = zGlobals.appSettings.https_app_url+"customer/order/status?cartid="+orderSessionID+"&amp;orderid="+data.value,'orderStatus'}
					);
