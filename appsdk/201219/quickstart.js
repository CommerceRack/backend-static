...
//if this is a custom extension and you are loading system extensions (prodlist, etc), then load ALL templates you'll need here.
		...
		"dependencies" : ['store_prodlist','store_prodlist','store_navcats','store_product','convertSessionToOrder','store_search','store_cart','store_crm'] //a list of other extensions (just the namespace) that are required for this one to load.  needs to be the extension namespace, so checkout_passive is convertSessionToOrder
		},
...
//				myControl.util.dump("BEGIN store_navcats.renderFormats.subcatList");
				...
				var subcatDetail = data.bindData.cleanValue;
				var o = '';
...
//maybe have the $tag binddata updated with the datapointer? 
			productSearch : function($tag,data)	{
				...
				var templateID = data.value['_rtag']['templateID'] ? data.value['_rtag']['templateID'] : 'productListTemplateResults';
				var pid;
...
//			myControl.util.dump(" -> Elastic-native namespace");
			...
			if(searchID)	{
				numRequests += myControl.ext.store_search.calls.appPublicProductSearch.init(jQuery.parseJSON(attribute),{'datapointer':'appPublicSearch|'+searchID,'templateID':bindData.loadsTemplate});
