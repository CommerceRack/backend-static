...
//currently, only IE isn't compatible.
		whatAjaxDataType2Use : function()	{
			myControl.util.dump("BEGIN model.whatAjaxDataType2Use");
			...
			if(navigator.appName == 'Microsoft Internet Explorer' && Number(jQuery.browser.version) < 10)	{
				myControl.util.dump(" -> ##### match for IE < 10 #####");
				...
				myControl.util.dump(" -> rootURL: "+rootUrl);
				myControl.util.dump(" -> JQUrl: "+JQUrl);
				if(JQUrl != rootUrl)	{
					r = 'jsonp';
					}
				}
