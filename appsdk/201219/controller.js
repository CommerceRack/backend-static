...
//check for ? to avoid js error which results when no uri params are present
			if(string || url.indexOf('?') > 0)	{
				url = url.replace('?', '').replace(/\&amp;/g,'&'); //on a uri, & could be encoded.
//				myControl.util.dump(" -> decoded uri: "+decodeURIComponent(url));
...
//an attibute will be set for each. {data-pid:PID} would output data-pid='PID'
		...
		translateElement : function(selector,data)	{
//			myControl.util.dump("BEGIN controller.renderFunctions.translateTemplate");
...
//					myControl.util.dump(' -> parsing order data. % attribute = '+attributeID);
					if(attributeID.substring(0,4) == 'data' && data['data'])	{
						value = data['data'][attributeID.substr(5)];
						}
					else if(attributeID.substring(0,8) == 'payments' && data['payments'])	{
						value = data['payments'][attributeID.substr(9)];
						}
					else if(attributeID.substring(0,12) == 'full_product' && data['full_product'])	{
...
//			var csv = data.bindData.cleanValue.split(',');
			var L = whitelist.length;
			var tagsDisplayed = 0;
...
//				myControl.util.dump("data.bindData.cleanValue.indexOf(whitelist[i]): "+data.bindData.cleanValue.indexOf(whitelist[i]));
				if(data.bindData.cleanValue.indexOf(whitelist[i]) >= 0 && (tagsDisplayed <= maxTagsShown))	{
