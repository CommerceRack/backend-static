...
#		next if ($file eq 'search_results');
		...
		if (not defined $configel) {
		...
		elsif (defined $configel->{'THEME_OLD'}) {
			# die();	# hmm.. already configured.
			}
		else {
			$changed++;
			print "THEME: ".$configel->{'THEME'};
			...
			$iniref = &TOXML::CSS::cssvar2iniref( $cssref );
			$configel->{'THEME'} = &ZTOOLKIT::buildparams( $iniref );
			print Dumper($configel);
			}
